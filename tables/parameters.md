...
| `orphan_method` or `om`             | `str`                 | `None`           | Method to handle orphan nucleotides: `integer`, `percentile`, `adaptive_1`, `adaptive_2`, `adaptive_3`, or `None`. `adaptive_1` uses block size and modification-budget checks; `adaptive_2` also requires flanking gaps longer than the dynamic threshold; `adaptive_3` also prevents modification when another orphan block with the same string occurs at the same position. |
